...
/// CharChooser allows you to randomly select a character based on provided weightings.
/// 
/// Instances are obtained with `Counter<MaybeChar, u32>::from()`.
...
/// 
/// Instances are obtained with `CharCounter::from()`.
/// 
/// Once you have an instance, you can call `word(min_len, max_len)`,
/// and a word matching those criteria will be generated for you.
#[derive(Serialize, Deserialize)]
...
impl CharkovChain {
    /// Generate a random word from the data in this chain.
    /// 
    /// Words are guaranteed not to be in the dictionary of words known to this chain,
    /// and the length will be greater than or equal to `min_len` and less than or equal to `max_len`.
    ...
    /// Internal method that generates a random word.
    /// 
    /// The generated word may not match the desired criteria,
    /// meaning that it may be a "real" word, generated by accident,
    /// or it may be of unacceptable length.
